...
{{- $domain := .Values.domain -}}
{
  ...
  "clients": [
    ...
    {{- $clientSecret := include "deriveSecret" (dict "seed" $seed "clientId" "people-oidc-client") -}}
    {{- $clients = append $clients (printf `{"clientId": "people", "name": "people", "enabled": true, "clientAuthenticatorType": "client-secret", "secret": "%s", "redirectUris": ["https://people.%s/*"], "webOrigins": ["https://people.%s"], "standardFlowEnabled": true, "implicitFlowEnabled": false, "directAccessGrantsEnabled": false, "serviceAccountsEnabled": false, "publicClient": false, "frontchannelLogout": true, "protocol": "openid-connect", "attributes": {"post.logout.redirect.uris": "https://people.%s/*", "user.info.response.signature.alg": "RS256"}, "defaultClientScopes": ["web-origins", "acr", "roles", "profile", "email"], "optionalClientScopes": ["address", "phone", "offline_access"]}` $clientSecret $domain $domain $domain) -}}
    {{- end -}}
    ...
    {{- $clientSecret := include "deriveSecret" (dict "seed" $seed "clientId" "find-oidc-client") -}}
    {{- $clients = append $clients (printf `{"clientId": "find", "name": "find", "enabled": true, "clientAuthenticatorType": "client-secret", "secret": "%s", "redirectUris": ["https://find.%s/*"], "webOrigins": ["https://find.%s"], "standardFlowEnabled": true, "implicitFlowEnabled": false, "directAccessGrantsEnabled": false, "serviceAccountsEnabled": true, "publicClient": false, "frontchannelLogout": true, "protocol": "openid-connect", "attributes": {"post.logout.redirect.uris": "https://find.%s/*", "user.info.response.signature.alg": "RS256"}, "defaultClientScopes": ["web-origins", "acr", "roles", "profile", "email"], "optionalClientScopes": ["address", "phone", "offline_access"]}` $clientSecret $domain $domain $domain) -}}
    {{- end -}}
    {{ join "," $clients }}
  ],
