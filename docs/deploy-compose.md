...
> **Bind mount permissions:** Bitnami images (PostgreSQL, Redis) run as UID 1001 and may fail with `permission denied` on host-mounted data directories owned by your user (UID 1000). This is handled automatically on first run.
